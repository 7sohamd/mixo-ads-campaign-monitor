      ...
      <div className="space-y-8">
        ...
        <section>
          <h3 className="text-sm font-bold text-slate-900 mb-6 flex items-center gap-2">
            Actionable Insights
            <span className="px-2 py-0.5 bg-indigo-50 text-indigo-600 text-[10px] font-bold rounded-md border border-indigo-200">
              AI
            </span>
          </h3>
          <div className="space-y-6">
            {loading ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center space-y-3 animate-in fade-in duration-500">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <p className="text-sm font-semibold text-slate-700 animate-in slide-in-from-bottom-2 duration-300" key={loadingMessageIndex}>
              ...
              <div className="flex items-center justify-center gap-2 py-8 animate-in zoom-in fade-out duration-300">
                ...
                <p className="text-base font-bold text-emerald-600">Insights ready!</p>
              </div>
            ) : (
              advice.map((item, i) => (
                <div
                ...
                >
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${i === 0 ? 'bg-indigo-50 text-indigo-600' : i === 1 ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'
                    }`}>
                    {i === 0 ? <TrendingUp size={12} /> : i === 1 ? <Sparkles size={12} /> : <AlertCircle size={12} />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[15px] font-semibold text-slate-900 leading-tight mb-1">
                      {item.title}
                    </p>
                    <p className="text-[13px] text-slate-600 leading-relaxed">
                      {item.advice}
                    </p>
                    <p className="text-[10px] text-slate-300 mt-1.5 uppercase font-bold tracking-widest">Just now</p>
                  </div>
                </div>
              ))
            )}
