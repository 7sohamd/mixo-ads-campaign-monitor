...
export const CampaignDetailModal: React.FC<CampaignDetailModalProps> = ({
    ...
    <div className="fixed inset-0 z-[100] flex justify-end bg-slate-900/10 backdrop-blur-[2px]" onClick={onClose}>
        <div
            className="w-full max-w-xl bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300 border-l border-slate-200"
            onClick={e => e.stopPropagation()}
        >
            <div className="p-8 pb-4 flex items-start justify-between">
                <div>
                    <h4 className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1">Detailed Analytics</h4>
                    <h2 className="text-2xl font-bold text-slate-900">{campaign.name}</h2>
                    <div className="flex items-center gap-3 mt-2">
                        <span className="px-2 py-0.5 bg-slate-100 text-[10px] font-bold text-slate-600 rounded-full uppercase tracking-tight">
                            ID: {campaign.id}
                        </span>
                ...
                <div className="relative group">
                    <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-full transition-colors relative z-10">
                        <X className="w-6 h-6 text-slate-400" />
                    </button>
                    <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-slate-900 text-white text-[10px] px-2 py-1 rounded font-bold pointer-events-none shadow-lg">
                        esc
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 pt-4 space-y-6">
                ...
                {isLoading ? (
                    <div className="space-y-6 text-center py-20">
                        ...
                        <p className="text-slate-500 text-sm font-medium">Synthesizing insights...</p>
                    </div>
                ) : insights ? (
                    <>
